<?php
...

	{
		...
		function galerias_muestra_imagenes_desactivadas ( $galeria_id = 0 )
...

				{
					...
					$salida .= "<tr>
									...
									<td>  
										...
										<p><a href='index.php?q=galerias_edita&s=".$this->generales->encriptar($galerias["galerias_id"],"galeria")."' class='btn btn-warning btn-sm'>".(($_SESSION["permiso_contenido"] == 1)? "<i class='glyphicon glyphicon-picture'></i> Editar im&aacute;genes": "<i class='glyphicon glyphicon-zoom-in'></i>Ver" )."</a> ";

										if( intval($galerias["galerias_id"]) > 1 && $_SESSION["permiso_contenido"] == 1)

											$salida .= "<a href='#' data-url='index.php?q=galeria&e=".$this->generales->encriptar($galerias["galerias_id"],"galeria")."' class='btn btn-xs btn-danger btn-sm delgal' ><i class='glyphicon glyphicon-remove'></i> Eliminar</a></p>";
...

		{
			...
			if( empty( $datos["galeria_titulo"] ) && strlen(  $datos["galeria_titulo"] ) < 3 )
			...
			if( !$galeria_id = $this->db->db_query( $sql, 2 ) )
...

		{
			...
			if( empty( $datos["imagen_titulo"] ) && strlen(  $datos["imagen_titulo"] ) < 3 )
			...
			$vigencia_imagen = '0000-00-00';

			if( !empty($datos["imagen_vigencia"]) )

			{

				$arr_imagen_vigencia = explode("/", $datos["imagen_vigencia"]);

				$vigencia_imagen = $arr_imagen_vigencia[2]."-".$arr_imagen_vigencia[1]."-".$arr_imagen_vigencia[0];
			...
			$sql = sprintf("INSERT INTO 
									...
									galerias_imagenes (galerias_id, galerias_imagenes_url_imagen, galerias_imagenes_url, galerias_imagenes_titulo, galerias_imagenes_descripcion, galerias_imagenes_vigencia, galerias_imagenes_fecha, galerias_imagenes_status) 
						...
						$this->db->db_evita_sql_injection( $vigencia_imagen ),
...

		{
			...
			$sql = sprintf("UPDATE galerias_imagenes SET galerias_imagenes_url ='%s', galerias_imagenes_titulo ='%s', galerias_imagenes_descripcion ='%s', galerias_imagenes_vigencia ='%s', galerias_imagenes_fecha ='%s', galerias_imagenes_status = 1 %s WHERE galerias_imagenes_id = %d ",
		...
		function galeria_recupera_imagen_seleccionada ( $galeria_id = 0 )

		{
			...
			$sql_imagenes = sprintf("UPDATE galerias_imagenes SET galerias_imagenes_status = 1 WHERE galerias_imagenes_id = %d ",
