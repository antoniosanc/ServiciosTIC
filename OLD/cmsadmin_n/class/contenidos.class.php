<?
	...
	class contenidos
	{
		...
		function contenidos_guarda_contenido( $datos = "")
		{
			...
			if( $datos["contenido_ficha"]['type'] == 'application/pdf' ) //verificamos que sea PDF
			{ 
	
				if( $datos["contenido_ficha"]['size'] < 5242880 ) //Verificamos que no pese más de 5Mb 5*1024*1024
				{
					$nombre_pdf = $datos["contenido_ficha"]['name'];
					$directorio_destino =  URL_USERFILES."file/";
					$cadena_unica = $this->generales->genera_cadena_aleatoria(4);
					
					$nombre_archivo = $directorio_destino.$cadena_unica."-".$nombre_pdf;

					if (move_uploaded_file($datos['contenido_ficha']['tmp_name'],$_SERVER["DOCUMENT_ROOT"].$nombre_archivo))
						$url_cv_pdf = $nombre_archivo;
				}
			}
			
			
				
			$sql = sprintf("UPDATE contenido SET contenido_titulo = '%s', contenido_descripcion = '%s', contenido_contenido = '%s', contenido_amenidades = '%s', contenido_mapa = '%s', contenido_plano = '%s', contenido_ficha = '%s', contenido_fecha_modificacion  = '%s',  contenido_btn_compartir = '%s', contenido_palabras_clave  = '%s'
								WHERE contenido_id = %d",
								...
								$this->db->db_evita_sql_injection(  $datos["contenido_editor_amenidades"] ),
								$this->db->db_evita_sql_injection(  $datos["contenido_editor_mapa"] ),
								$this->db->db_evita_sql_injection(  $datos["contenido_editor_plano"] ),
								$this->db->db_evita_sql_injection(  $url_cv_pdf ),								
								$this->generales->hoy(),
								$compartir_rs,
				...
				foreach( $arr_seccion as $seccion )
				{
					...
					$salida .= "	</td>
									...
									<td>  
										...
										<a title='Restaurar informaci&oacute;n' href='index.php?q=contenidos_restaura&s=".$this->generales->encriptar($seccion["contenido_historia_id"],"contenidos")."' class='btn btn-success btn-sm'><i class='glyphicon glyphicon-download-alt'></i></a>
