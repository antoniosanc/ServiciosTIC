	...
	class configuracion
	{
		...
		function configuracion_obten_configuracion_miscelaneo ()
		{
			$sql = sprintf("SELECT 
								...
								c.configuracion_estilo_personalizado,
		...
		function configuracion_guarda_configuracion_miscelaneo ( $datos = "")
		{
			...
			$sql = sprintf("UPDATE configuracion SET configuracion_codigo_analytics = '%s', configuracion_keywords_base = '%s', configuracion_descripcion_base = '%s', configuracion_estilo_personalizado = '%s', configuracion_ultima_modificacion  = '%s', usuarios_id = %d WHERE configuracion_id = 1 AND configuracion_status = 1",
				...
				$this->db->db_evita_sql_injection( $this->generales->limpia_caracteres_to_ascii( $datos["css_custom"]) ),
				$this->generales->hoy(),
				$_SESSION["uid"]
			);
				...
				foreach( $arr_usuarios as $lista_usuarios => $usuarios)
				{
					...
					if( intval( $usuarios["usuarios_es_admin"]) == 0)
					{
						$html_salida .=  '<td align="center">  
												<a href="index.php?q=usuarios_edita&i='.$this->generales->encriptar($usuarios["usuarios_id"],"usuario").'" class="btn btn-mini btn-success">Editar</a> &nbsp;
												<a href="index.php?q=configuracion&eu='.md5(1).'&i='.$this->generales->encriptar($usuarios["usuarios_id"],"usuario").'" class="btn btn-mini btn-danger">Eliminar</a> 
										</td>';
					}
					else
					{
		...
		function configuracion_guarda_permisos_usuarios ( $datos = 0 )
		{
			...
			$sql = sprintf("INSERT INTO usuarios_permisos(
									...
									VALUES (%d, '%s', '%s', '%s', '%s', '%s', '%s', %d, 1)
								ON DUPLICATE KEY UPDATE
									usuarios_id = %d, 
									usuarios_permisos_secciones = '%s', 
									usuarios_permisos_contenido = '%s', 
									usuarios_permisos_estadisticas = '%s', 
									usuarios_permisos_centro_negocios = '%s', 
									usuarios_permisos_configuracion = '%s',
									usuarios_permisos_fecha_modificacion = '%s',
									usuarios_id_modifico = '%s'",
		...
		function configuracion_guarda_nuevo_usuario ( $datos = 0)
		{
			...
			$sql_insert_usuario = sprintf("INSERT INTO usuarios(usuarios_nombre, usuarios_usuario, usuarios_password, usuarios_email, usuarios_fecha_creacion, usuario_inserta_id, usuarios_status) VALUES ('%s','%s','%s','%s','%s','%s',1) ",
